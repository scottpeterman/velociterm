...
"""
routes/auth.py
Authentication and Authorization Routes with JWT Support - FIXED
"""
...
def create_auth_routes(auth_manager: AuthenticationManager, connection_handlers: ConnectionHandlers):
    ...
    async def login(request: Request):
        ...
        try:
            ...
            if auth_result.success:
                ...
                ssh_key = connection_handlers.workspace_manager.load_ssh_key(auth_result.username)
                if ssh_key:
                    connection_handlers.set_user_ssh_key(auth_result.username, ssh_key)
                    logger.info(f"Loaded SSH key for {auth_result.username}")
                else:
                    logger.info(f"No SSH key found for {auth_result.username}")
    ...
    async def login_for_access_token(request: Request):
        ...
        try:
            ...
            if auth_result.success:
                ...
                # ✅ FIXED: Load SSH key if available (use connection_handlers, not auth_manager)
                ssh_key = connection_handlers.workspace_manager.load_ssh_key(auth_result.username)
                if ssh_key:
                    connection_handlers.set_user_ssh_key(auth_result.username, ssh_key)
                    logger.info(f"Loaded SSH key for {auth_result.username}")
                ...
                logger.info(f"JWT token login successful for {auth_result.username}")
                return response

            else:
                logger.warning(f"JWT token login failed for {username}: {auth_result.error}")
                return JSONResponse(
                    status_code=401,
                    content={
    ...
    @router.get("/status")
    async def auth_status(request: Request, session: str = Cookie(None)):
        """Get current authentication status - checks if user is authenticated"""
        try:
            # Try to get current user without throwing exception
            username = None
            auth_method = None
            groups = []
    ...
    async def logout(
        ...
        connection_handlers.clear_user_ssh_key(current_user.username)
