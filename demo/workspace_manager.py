...
class WorkspaceManager:
    ...
    def load_ssh_key(self, username: str) -> Optional[bytes]:
        """Load user's default SSH private key if it exists (MVP: single key support)"""
        try:
            user_dir = self._get_user_dir(username)
            key_dir = user_dir / "ssh_key"

            # Try common private key names in order of preference
            key_filenames = ["id_rsa", "id_ed25519", "id_ecdsa", "id_dsa"]

            for key_filename in key_filenames:
                key_path = key_dir / key_filename
                if key_path.exists():
                    logger.info(f"Found SSH key for {username}: {key_filename}")
                    with open(key_path, 'rb') as f:
                        key_data = f.read()

                    # Basic validation: check if it looks like a private key
                    if b'PRIVATE KEY' in key_data:
                        logger.info(f"Loaded SSH key for {username} ({len(key_data)} bytes)")
                        return key_data
                    else:
                        logger.warning(f"File {key_filename} doesn't appear to be a valid private key")

            logger.debug(f"No SSH key found for {username}")
            return None

        except Exception as e:
            logger.error(f"Failed to load SSH key for {username}: {e}")
            return None
